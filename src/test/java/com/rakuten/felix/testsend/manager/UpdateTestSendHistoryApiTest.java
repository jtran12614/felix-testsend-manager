...
@ActiveProfiles("test")
public class UpdateTestSendHistoryApiTest extends AbstractIntegrationTest {
    @Inject
    private MockMvc mvc;
    ...
    public void initializeTestSendHistoryTest(String name, TestData testData) throws Exception {
        ...
        val testHistoryId = apiData.request.params.get("testHistoryId");
        apiData.request.headers.forEach((key, value) -> headers.add(key, String.valueOf(value)));
        val expected = mvc.perform(MockMvcRequestBuilders.post("/testsend-manager/api/v1/update-history/{testHistoryId}", testHistoryId)
                        .headers(headers)
    ...
    public static Stream<Arguments> provideTestData() throws IOException {
        ...
        val testCaseFile = UpdateTestSendHistoryApiTest.class.getClassLoader().getResource("test-cases/update-test-send-history.json");
        val testCases = mapper.readValue(testCaseFile, new TypeReference<List<UpdateTestSendHistoryApiTest.TestData>>() {
        });
